...
### Moat（3つの堀）【2026-03-03 修正】
1. ITIL-Native AI（ガバナンスをコード化したAI）
2. WebOps Agent技術による物理レイヤー接続（APIのないレガシーシステムも自律化）
3. Cyber-Physical Data Loop（コーポレートナレッジの複利蓄積）

### KDDIまとめてオフィスの実態【重要：認識修正済み】
- KDDIは「独占的パートナー」「販売チャネル」ではなく、**L4物理実行の仕入れ先（下請け）**
- KDDIまとめてオフィスにAPIは存在しない → WebOps Agent（Playwright）でブラウザ自動化
- L4アーキテクチャは複数の物流・BPO事業者に対応できる抽象化レイヤーとして設計
- 「独占的API連携」という表現は全文書から削除済み
